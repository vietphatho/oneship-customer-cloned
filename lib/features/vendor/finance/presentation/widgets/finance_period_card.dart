...
class VendorFinancePeriodCard extends StatelessWidget {
  const VendorFinancePeriodCard({
    super.key,
          ...
          children: [
            ...
            Row(
              children: [
                ...
                TextButton(onPressed: onTap, child: Text('view_details'.tr())),
              ],
            ),
...
class _PeriodTimeline extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final currentStep = switch (period.periodStatus) {
      SettlementStatus.open => 1,
      SettlementStatus.approved => 2,
      SettlementStatus.locked => 3,
      SettlementStatus.cancelled || SettlementStatus.all => 1,
    };
    final steps = [
      'timeline_ongoing'.tr(),
