...
extension OrdersStateX on OrdersState {
  ...
  List<OrdersHistoryEntity> get filteredAllOrdersHistoryList =>
      _applyHistoryFilter(allOrdersHistoryList);
  List<OrdersHistoryEntity> get filteredDeliveredOrdersHistoryList =>
      _applyHistoryFilter(deliveredOrdersHistoryList);
  List<OrdersHistoryEntity> get filteredReturnedOrdersHistoryList =>
      _applyHistoryFilter(returnedOrdersHistoryList);

  List<OrdersHistoryEntity> _applyHistoryFilter(
    List<OrdersHistoryEntity> list,
  ) {
    if (processingOrdersFilters.isEmpty) return list;
    return list.where((order) {
      final filters = processingOrdersFilters;
  ...
  int getHistoryOrdersCountForStatus(OrderStatus status) {
    if (status == OrderStatus.allProcessing) {
      return filteredAllOrdersHistoryList.length;
    }
    if (status == OrderStatus.delivered) {
      return filteredDeliveredOrdersHistoryList.length;
    }
    if (status == OrderStatus.returned) {
      return filteredReturnedOrdersHistoryList.length;
    }
    return 0;
  }
}
